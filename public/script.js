...
// Migrate old localStorage groups format { name: members[] } to new { id: { name, members, isLegacy } }
(function migrateGroupsFormat() {
  const keys = Object.keys(groups);
  if (keys.length === 0) return;
  // Detect old format: first value is an array (members list)
  const firstVal = groups[keys[0]];
  if (Array.isArray(firstVal)) {
    const migrated = {};
    keys.forEach((name) => {
      migrated[name] = { name, members: groups[name], isLegacy: true };
    });
    groups = migrated;
...
function scanReceipt() {
  ...
  // Add country parameter based on tax profile
  const taxProfile = localStorage.getItem("taxProfile") || "singapore";
  const country = taxProfile === "malaysia" ? "MY" : "SG";
...
function editDish(index) {
  ...
  dish.members.forEach((member) => {
    ...
    if (typeof member === "object" && member.name) {
      ...
      Object.values(storedGroups).forEach((groupObj) => {
        const membersList = Array.isArray(groupObj) ? groupObj : (groupObj.members || []);
...
function updateDishList() {
  ...
  if (!dishList) {
    return;
  } // Add early return if dishList is null
...
function showFinaliseSettleBillScreen() {
  ...
  function showLoadingScreen() {
    ...
    const billData = {
      ...
      serviceChargeValue,
      birthdayPerson,
      groupName: (currentGroup && groups[currentGroup]) ? groups[currentGroup].name : currentGroup || null,
      groupId: (currentGroup && groups[currentGroup] && !groups[currentGroup].isLegacy) ? currentGroup : null,
    };

    const submitUrl = editBillId ? `/api/bills/${editBillId}` : "/calculate";
    const submitMethod = editBillId ? "PUT" : "POST";

    fetchWithUserId(submitUrl, {
      method: submitMethod,
      ...
      `;
        ...
        // Show and wire up edit bill button on success screen
        const editBillBtn = document.querySelector(".edit-bill-btn");
        if (editBillBtn) {
          editBillBtn.style.display = "";
        ...
        editBillId = null;
        const settleLahBtn = document.querySelector(".settle-lah-btn");
        if (settleLahBtn) {
          settleLahBtn.textContent = "SettleLah!";
        }
      })
      .catch((err) => {
        clearInterval(loadingInterval);
...
function initializeBillSummaryDrag() {
  ...
  function handleDragMove(e) {
    ...
    if (e.type === "mousemove" && !isMouseDrag) {
      return;
    }
...
function initializeSwipeGesture() {
  ...
  const handleTouchMove = function (e) {
    ...
    if (touchData.isScrolling || touchData.isProcessingGesture) {
      return;
    }
  ...
  const handleTouchEnd = function (e) {
    ...
    if (touchData.isProcessingGesture || touchData.isScrolling) {
      return;
    }
    ...
    if (touchDuration < MIN_TOUCH_DURATION || touchDuration > MAX_TOUCH_DURATION) {
      return;
    }
...
function updateGroupSelect() {
  ...
  Object.entries(groups).forEach(([groupId, groupObj]) => {
    ...
    label.appendChild(document.createTextNode(groupObj.name || groupId));
      ...
      setTimeout(() => {
        ...
        data.bills
          ...
          .forEach((bill) => {
            ...
            settleItem.innerHTML = `
              <div class="settle-top">
                ...
                <div class="settle-actions">
                  <button class="edit-history-btn" data-bill-id="${bill.id}" title="Edit">
            ...
            const editBtn = settleItem.querySelector(".edit-history-btn");
            if (editBtn) {
              editBtn.addEventListener("click", function (e) {
                e.preventDefault();
                e.stopPropagation();
                const editBillId = this.getAttribute("data-bill-id");
                window.location.href = `/?edit=${editBillId}`;
              });
            }
...
function updateTaxRates(profile) {
  ...
  // Update the visual tax profile selector
  updateTaxProfileSelector(profile);

  // Legacy support - only update if elements exist (for backward compatibility)
...
// Initialize the new tax profile selector
function initTaxProfileSelector() {
  const profileOptions = document.querySelectorAll(".tax-profile-option");

  profileOptions.forEach((option) => {
    option.addEventListener("click", function () {
      const profile = this.dataset.profile;

      // Update localStorage and rates
      localStorage.setItem("taxProfile", profile);
      updateTaxRates(profile);

      // Update the hidden select for backward compatibility
      const taxProfileSelect = document.getElementById("taxProfile");
      if (taxProfileSelect) {
        taxProfileSelect.value = profile;
      }
  ...
  // Initialize the visual state based on current selection
  const currentProfile = localStorage.getItem("taxProfile") || "singapore";
  updateTaxProfileSelector(currentProfile);
}

// Update the visual state of the tax profile selector
function updateTaxProfileSelector(profile) {
  const profileOptions = document.querySelectorAll(".tax-profile-option");
...
function showSettleView(viewId) {
  ...
  // Reset bill-specific country when going back to earlier views
  if (viewId === "settleChoiceView" || viewId === "addMembersView") {
    resetBillSpecificCountry();
  }
    ...
    if (data.groups) {
      ...
      if (Array.isArray(data.groups)) {
        data.groups.forEach(g => {
          groups[g.id] = { name: g.name, members: g.members, isLegacy: g.isLegacy };
        });
      } else {
        // Fallback: old object format { name: members[] }
        Object.keys(data.groups).forEach(key => {
          const val = data.groups[key];
          if (Array.isArray(val)) {
            groups[key] = { name: key, members: val, isLegacy: true };
          } else {
            groups[key] = val;
          }
        });
...
function renderSavedGroups() {
  ...
  if (groupsList.length === 0) {
    ...
    groupsList.forEach((groupId, index) => {
      const groupObj = groups[groupId];
      const card = createSavedGroupCard(groupId, groupObj.name, groupObj.members, index);
...
function createSavedGroupCard(groupId, groupName, membersList, cardIndex) {
...
function selectSavedGroup(groupId) {
  ...
  const selectedCard = document.querySelector(`.saved-group-card[data-group-id="${groupId}"]`);
  if (selectedCard) {
...
function editSavedGroup(groupId, event) {
  ...
  if (groupNameContainer && groupNameInput) {
    ...
    groupNameInput.value = groups[groupId].name;
...
function deleteSavedGroup(groupId, event) {
  if (event) {
    event.stopPropagation();
  }

  // Store the group ID for the confirmation
  document.getElementById("deleteGroupModal").dataset.groupId = groupId;
...
function showGroupMembersView(groupId) {
...
function saveGroupToStorage() {
  if (currentGroup) {
    ...
    let groupName = groups[groupId] ? groups[groupId].name : groupId;
    ...
    if (isEditingGroup) {
      ...
      if (groupNameInput) {
        ...
        const nameConflict = Object.entries(groups).some(
          ([id, g]) => id !== groupId && g.name === newGroupName
        );
        if (nameConflict) {
          if (errorElement) {
    ...
    // For legacy groups being edited, they'll get a new UUID from the server
    const isLegacy = groups[groupId] ? groups[groupId].isLegacy : false;

    groups[groupId] = { name: groupName, members: [...members], isLegacy };
    localStorage.setItem("groups", JSON.stringify(groups));

    // Save to Firebase — pass groupId (null for legacy so server generates new UUID)
    saveGroupToFirebase(isLegacy ? null : groupId, groupName, [...members], isLegacy ? groupId : null);
  }
}

// Function to save the group to Firebase
async function saveGroupToFirebase(groupId, groupName, groupMembers, oldLegacyId) {
  try {
    ...
    // If server returned a new groupId, update local reference
    const localKey = oldLegacyId || groupId;
    if (result.groupId) {
      // Find the local key to replace (could be legacy ID, existing ID, or temp ID)
      let keyToReplace = localKey;
      if (!keyToReplace || !groups[keyToReplace]) {
        // Find temp key matching this group name
        keyToReplace = Object.keys(groups).find(
          k => k.startsWith('_new_') && groups[k].name === groupName
        );
      }

      if (keyToReplace && keyToReplace !== result.groupId && groups[keyToReplace]) {
        groups[result.groupId] = { ...groups[keyToReplace], isLegacy: false };
        delete groups[keyToReplace];
        if (currentGroup === keyToReplace) {
          currentGroup = result.groupId;
        }
        localStorage.setItem('groups', JSON.stringify(groups));
      }
    }

    // If upgrading a legacy group, delete the old Firestore doc
    if (oldLegacyId && result.groupId !== oldLegacyId) {
      deleteGroupFromFirebase(oldLegacyId);
    }
  } catch (error) {
...
async function deleteGroupFromFirebase(groupId) {
  try {
    const response = await fetchWithUserId(`/api/groups/${encodeURIComponent(groupId)}`, {
      method: "DELETE",
    });
    ...
    if (!response.ok) {
      ...
      // console.log(`Group deleted from Firebase successfully`);
    }
  } catch (error) {
...
function initPageNavigation() {
  ...
  document.querySelector(".get-started-btn").addEventListener("click", function () {
    ...
    if (savedGroupSelected) {
      ...
      if (title) {
        title.textContent = "Settle With New Group";
      }
  ...
  document.querySelectorAll(".next-btn, .new-group-next-btn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (isEditingGroup && currentGroup) {
        ...
        // Store the group ID for the confirmation
        document.getElementById("groupUpdatedModal").dataset.groupId = currentGroup;
  ...
  document.querySelector(".saved-groups-next-btn").addEventListener("click", function () {
    ...
    if (newFavouriteGroupBtn && successMessage && groupNameDisplay) {
      ...
      const displayName = groups[currentGroup] ? groups[currentGroup].name : currentGroup;
      groupNameDisplay.textContent = `Group "${displayName}" selected`;
...
if (confirmGroupBtn) {
  confirmGroupBtn.addEventListener("click", function () {
    ...
    const nameExists = Object.values(groups).some(g => g.name === groupName);
    if (nameExists) {
      inputField.classList.add("error");
    ...
    // Create new group with a temporary ID (server will assign UUID)
    const tempId = '_new_' + Date.now();
    groups[tempId] = { name: groupName, members: [...members], isLegacy: false };
    localStorage.setItem("groups", JSON.stringify(groups));
    currentGroup = tempId;

    // Save to Firebase — null groupId so server generates UUID
    saveGroupToFirebase(null, groupName, [...members]);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("taxProfile").addEventListener("change", function () {
    ...
    // Update the visual tax profile selector
    updateTaxProfileSelector(taxProfile);
  });

  // Add event listeners for the new tax profile selector
  initTaxProfileSelector();
...
async function loadBillForEditing(billId) {
  try {
    const response = await fetchWithUserId(`/result/${billId}`);
    if (!response.ok) {
      console.error("Failed to load bill for editing");
      return;
    }
    ...
    // Clear existing members and dishes
    members = [];
    dishes = [];
    ...
    if (data.members && data.members.length > 0) {
      data.members.forEach((member) => {
        members.push({ name: member.name, avatar: member.avatar || 1 });
      });
    }
    ...
    // Set settle matter
    const settleMatterEl = document.getElementById("settleMatter");
    if (settleMatterEl && data.settleMatter) {
      settleMatterEl.value = data.settleMatter;
    }

    // Set tax profile
    if (data.taxProfile) {
      const taxProfileEl = document.getElementById("taxProfile");
      if (taxProfileEl) {
        taxProfileEl.value = data.taxProfile;
        updateTaxRates(data.taxProfile);
        updateTaxProfileSelector(data.taxProfile);
      }
    }
    ...
    // Set service charge
    const serviceChargeCheckbox = document.getElementById("serviceChargeCheckbox");
    if (serviceChargeCheckbox) {
      serviceChargeCheckbox.checked = !!data.applyServiceCharge;
    }
    const serviceChargeValueEl = document.getElementById("serviceChargeValue");
    if (serviceChargeValueEl && data.serviceChargeValue) {
      serviceChargeValueEl.value = data.serviceChargeValue;
    }

    // Set GST
    const gstCheckbox = document.getElementById("gstCheckbox");
    if (gstCheckbox) {
      gstCheckbox.checked = !!data.applyGst;
    }

    // Set PayNow details
    const paynowNameEl = document.getElementById("paynowName");
    if (paynowNameEl && data.paynowName) {
      paynowNameEl.value = data.paynowName;
    }
    const paynowPhoneEl = document.getElementById("paynowPhone");
    if (paynowPhoneEl && data.paynowID) {
      paynowPhoneEl.value = data.paynowID;
    }
    ...
    // Restore group context if bill was created from a group
    if (data.groupId) {
    ...
    // Update the settle button text
    const settleLahBtn = document.querySelector(".settle-lah-btn");
    if (settleLahBtn) {
      settleLahBtn.textContent = "Update Bill";
    }

    // Navigate to the settle flow with the add items view
    showSettleNowScreen();
    showSettleView("addSettleItemView");

    // Populate member bubbles in the add-item view so users can assign dishes
    updateSettleItemMembersUI();

    // Hide the scan receipt button in edit mode
    const scanItemBtn = document.querySelector(".scan-item-btn");
    if (scanItemBtn) {
      scanItemBtn.style.display = "none";
    }

    // Set previousView so the back button navigates home instead of a blank page
    previousView = "settleChoiceView";

    // Update dish summary - open fully in edit mode so user sees all items
    if (dishes.length > 0) {
      showSummary("fully-open");
    }
...
function showSavedGroupsModal() {
  ...
  if (groupsList.length === 0) {
    ...
    groupsList.forEach((groupId) => {
      ...
      const displayName = groupObj.name || groupId;
      const groupItem = document.createElement("div");
      groupItem.className = "group-item";
      groupItem.innerHTML = `
        <div class="group-item-name">${displayName}</div>
...
async function updateLastCreatedGroup() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.groups) {
        ...
        if (Array.isArray(data.groups)) {
          data.groups.forEach(g => {
            localGroups[g.id] = { name: g.name, members: g.members, isLegacy: g.isLegacy };
          });
        } else {
          ...
          Object.keys(data.groups).forEach(key => {
            const val = data.groups[key];
            if (Array.isArray(val)) {
              localGroups[key] = { name: key, members: val, isLegacy: true };
            } else {
              localGroups[key] = val;
            }
          });
  ...
  const lastGroupId = groupIds[0];
  const lastGroupObj = localGroups[lastGroupId];
  const lastGroupMembers = lastGroupObj.members;
  ...
  setTimeout(() => {
    ...
    const groupHeader = groupCard.querySelector(".card-header p");
    if (groupHeader) {
      groupHeader.textContent = lastGroupObj.name || lastGroupId;
    }

    // Update avatar group with actual members
    const avatarGroup = groupCard.querySelector(".avatar-group");
    if (avatarGroup && lastGroupMembers && lastGroupMembers.length > 0) {
      avatarGroup.innerHTML = "";

      // Show up to 5 members
      const membersToShow = lastGroupMembers.slice(0, 5);
  ...
  fetchWithUserId("/api/history/latest", {
      ...
      setTimeout(() => {
        ...
        if (!data || !data.bill) {
          ...
          if (settleInfo) {
            ...
            if (amountEl) {
              amountEl.textContent = "No settlements yet";
            }
            if (matterEl) {
              matterEl.textContent = "Start settling bills with your friends";
            }
            if (groupEl) {
              groupEl.textContent = "Tap 'Settle Now' to begin";
            }
          }
    ...
    .catch((error) => {
      ...
      if (settleInfo) {
        const amountEl = settleInfo.querySelector(".settle-info-amount");
        if (amountEl) {
          amountEl.textContent = "Error loading";
        }
      }
    });
}
...
function syncTaxProfileDropdown() {
  ...
  // Initialize the new tax profile selector if it exists
  const profileOptions = document.querySelectorAll(".tax-profile-option");
...
// Bill-specific country override functionality
let billSpecificCountry = null;
...
function initCountryModal() {
  ...
  // Show modal when change country button is clicked
  changeCountryBtn?.addEventListener("click", () => {
    const currentProfile = billSpecificCountry || localStorage.getItem("taxProfile") || "singapore";

    // Update selected state
    countryOptions.forEach((option) => {
      option.classList.toggle("selected", option.dataset.country === currentProfile);
    });
  ...
  // Cancel button
  cancelBtn?.addEventListener("click", hideCountryModal);

  // Confirm button
  confirmBtn?.addEventListener("click", () => {
    if (selectedCountry) {
      setBillSpecificCountry(selectedCountry);
      hideCountryModal();
    }
  });
...
function setBillSpecificCountry(country) {
  billSpecificCountry = country;
  updateCountryDisplay();
  updateGSTCheckboxLabel(country);
  updatePaymentLabels(country);

  // Update the hidden tax profile select for backend compatibility
  const taxProfileSelect = document.getElementById("taxProfile");
  if (taxProfileSelect) {
    taxProfileSelect.value = country;
  }

  // Show feedback that it's bill-specific
  //  const changeBtn = document.getElementById('changeCountryBtn');
  //  if (changeBtn) {
  //     changeBtn.textContent = billSpecificCountry ? 'Change again' : 'Change for this bill only';
  //  }
}
...
function updateCountryDisplay() {
  ...
  if (currentCountryDisplay) {
    const country = billSpecificCountry || localStorage.getItem("taxProfile") || "singapore";
    const countryName = country === "singapore" ? "Singapore" : "Malaysia";
    const indicator = billSpecificCountry ? " (Bill Override)" : "";
    currentCountryDisplay.textContent = countryName + indicator;
  }
}

function resetBillSpecificCountry() {
  billSpecificCountry = null;
  updateCountryDisplay();

  // Reset to user's default setting
  const defaultProfile = localStorage.getItem("taxProfile") || "singapore";
  updateGSTCheckboxLabel(defaultProfile);
  updatePaymentLabels(defaultProfile);

  const taxProfileSelect = document.getElementById("taxProfile");
  if (taxProfileSelect) {
    taxProfileSelect.value = defaultProfile;
  }

  //  const changeBtn = document.getElementById("changeCountryBtn");
  //  if (changeBtn) {
  //     changeBtn.textContent = "Change for this bill only";
  //  }
}
...
window.onload = async function () {
  ...
  setTimeout(async () => {
    ...
    const taxProfileSelect = document.getElementById("taxProfile");
    if (taxProfileSelect) {
      taxProfileSelect.value = "singapore";
    }

    const serviceRate = document.getElementById("serviceRate");
    if (serviceRate) {
      serviceRate.textContent = "10%";
    }
...
function updateReceiptDetails(receiptContainer, data) {
  ...
  if (settleMatterEl) {
    settleMatterEl.textContent = data.settleMatter || "No one ask!";
  }
  if (dateEl) {
  ...
  if (itemCountEl) {
    itemCountEl.textContent = data.itemCount;
  }
  if (subtotalEl) {
