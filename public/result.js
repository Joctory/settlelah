              ...
              <div class="animate-staggered">
                    ...
                    <div class="member-bill-row">
                      ...
                      <span class="member-bill-row-value">
                        ${(() => {
                          ...
                          const numPeople =
                            item.sharedWith ?
                              (item.sharedWith.match(/<span class="shared-member">/g) || [])
                                .length + 1
                            : 1;
                ...
                ${
                  ...
                  (
                    ...
                    memberData[billData.birthdayPerson]
                  ) ?
                    `
                ...
                ${
                  ...
                  <div class="member-bill-row">
                    <span class="member-bill-row-label">GST (${
                      data.taxProfile === "singapore" ? "9%"
                      : data.taxProfile === "malaysia" ? "6%"
                      : "9%"
                    })</span>
            ...
            <div class="paynow-section modal-fade-in">
              ...
              ${
                ...
                <div class="paynow-qr-container" id="paynow-qr-container">
                  <img src="https://www.sgqrcode.com/paynow?mobile=${data.paymentInfo.phoneNumber.replace(
                    /[^0-9]/g,
                    ""
                  ...
                  <div class="member-bill-row">
                    ...
                    <span class="member-bill-row-value">
                      ${(() => {
                        ...
                        const numPeople =
                          item.sharedWith ?
                            (item.sharedWith.match(/<span class="shared-member">/g) || []).length +
                            1
                          : 1;
                        // Extract price value from string, remove "$" and convert to number
                        const price = parseFloat(item.price.replace("$", ""));
        ...
        <div class="paynow-qr-container" id="paynow-qr-container">
          <!-- QR code will be generated here -->
            <img src="https://www.sgqrcode.com/paynow?mobile=${data.paymentInfo.phoneNumber.replace(
              ...
              .replace(/-/g, "%2F")}%2023%3A59&ref_id=SettleLah${(
              memberDisplayName || "SettleLah"
            ).replace(/[^A-Za-z0-9]/g, "")}&company=" alt="PayNow QR Code" class="desktop-qr-code">
        </div>
...
async function updatePaymentStatus(memberName, hasPaid) {
  try {
    ...
    if (result.success) {
      ...
      showToast(
        hasPaid ? `${memberName} marked as paid!` : `${memberName} payment status updated!`
      );
    } else {
...
function updateMemberAvatarStatus(memberName, hasPaid) {
  const memberAvatars = document.querySelectorAll(
    `.member-avatar-wrapper[data-name="${memberName}"]`
  );
...
function getPaymentProgress() {
  ...
  const paidCount = membersWithAmount.filter(
    (member) => paymentStatus[member.name]?.hasPaid
  ).length;
...
function checkPaymentCompletion() {
  ...
  const nonPayerMembers = billData.members.filter(
    (member) =>
      ...
      member.name !== billData.birthdayPerson &&
      (billData.totals?.[member.name] ?? 0) > 0
  );
...
function handleShareBill() {
  ...
  const settleMatter =
    document.querySelector(".successSettleMatter")?.textContent || "SettleLah Bill";
