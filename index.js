...
app.post('/api/scan-receipt', upload.single('document'), async (req, res) => {
  try {
    ...
    const country = req.body.country || 'SG';
    ...
    if (!clientId || !username || !apiKey) {
      console.error('Missing Veryfi credentials');
      return res.status(500).json({ error: 'Server configuration error: Missing API credentials' });
    }

    // Create form data for the Veryfi API request
    ...
    // Make the API request to Veryfi
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: `Veryfi API error: ${response.status}`,
        details: errorText
      });
    }
    ...
    // Process the data from Veryfi response
    const newDishes = [];
    ...
    if (newDishes.length === 0 && data.total) {
      newDishes.push({
        name: data.vendor?.name || 'Receipt Item',
        cost: parseFloat(data.total),
        members: []
      });
    }
